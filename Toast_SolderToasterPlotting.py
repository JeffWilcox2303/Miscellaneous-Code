...
data = pd.read_csv('Solder Toaster Temperature Measurements - Toast Setting.csv')
...
dT = [(T[i+1] - T[i])/10 for i in range(23)]
Y = np.log(np.abs(dT))
print(Y)
...
print(f'Tf = {Tf}')
print(f'Tau = {tau}')
calcY = Tf - (Tf-Ti)*np.exp(-time/tau)
